...
// Расчет уровня, следующего уровня и прогресса
...
const initTelegram = () => {
  console.log('Проверка window:', window);
  ...
  console.log('initData:', initData || 'Пусто');

  if (!initData) {
    console.error('initData отсутствует');
    alert('Данные авторизации не получены. Перезапустите приложение.');
    return;
  }
  ...
  profile.value = {
    ...
    points: user.points || 0 // Предполагаем, что points может прийти, иначе 0
  };
